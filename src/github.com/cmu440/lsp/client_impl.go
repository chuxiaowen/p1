...
type client struct {
	...
	ackSlidingWindow []bool	// sliding window for ack messages
...
func NewClient(hostport string, params *Params) (Client, error) {
	...
	ackSlidingWindow := make([]bool, params.WindowSize)
	writePendingNum := 0
	ackPendingNum := 0
	...
	Client := &client{conn, connid, raddr, receiveseqid ,writeseqid, epochtimes, lastepoch, params, 
		...
		readReplyChan, writeReplyChan, readList, writeList, writeSlidingWindow, writeWindowIndicator, 
		ackSlidingWindow, writePendingNum, ackPendingNum, connectPending, networkStopFlag, appStopFlag}
	...
	//fmt.Println("write connect message")
	...
	if val != nil {
		...
		//fmt.Println("connid is: ",Client.connid)
		return Client, nil
	} else {
...
func (c *client) handleReceiveMessage(msg *Message) {
	...
	case MsgData:
		...
		if msg.SeqNum >= number && msg.SeqNum < number+c.parameter.WindowSize {
			...
			if !c.ackSlidingWindow[index] {
				// Insert into readList
				c.ackSlidingWindow[index] = true
...
func (c *client) shiftAckWindow(number int) {
	...
	if shift < c.parameter.WindowSize {
	for i := shift; i < c.parameter.WindowSize; i++ {
		c.ackSlidingWindow[i] = c.ackSlidingWindow[i-shift]
	}

	i := c.parameter.WindowSize - shift
	for i < c.parameter.WindowSize {
		c.ackSlidingWindow[i] = false
		i += 1
	}
	} else {
		for i:=0; i < c.parameter.WindowSize; i++ {
